    ...
    def fetch(self, word, config):
        ...
        try:
            ...
            if response.status_code != 200:
                self.message_callback(f"Failed to fetch data for '{word}' (HTTP {response.status_code}).")
                return []
        except requests.RequestException as e:
            self.message_callback(f"Network error for '{word}': {str(e)}")
            return []
        
        try:
            soup = BeautifulSoup(response.text, parser)
        except Exception as e:
            self.message_callback(f"Parsing error for '{word}': {str(e)}. Try setting PARSER to 'html.parser' or install 'lxml'.")
            return []
        ...
        if not main_card:
            self.message_callback(f"No entry found for '{word}'.")
            return []
        ...
        for key, field_idx in field_map.items():
            if field_idx >= 0:
                if key == 'word':
                    mapped_data[field_idx] = entry_word
                elif key == 'pronunciation':
                    mapped_data[field_idx] = pronunciation
                elif key == 'pos':
                    mapped_data[field_idx] = pos
                elif key == 'inflections':
                    mapped_data[field_idx] = inflections
                elif key == 'def_zh':
                    mapped_data[field_idx] = def_zh_str
                elif key == 'examples':
                    mapped_data[field_idx] = examples_str
                    ...
                    if a:
                        ...
                        if next_div:
                            ...
                            for key, field_idx in field_map.items():
                                if field_idx >= 0:
                                    if key == 'word':
                                        mapped_rel_data[field_idx] = rel_word
                                    elif key == 'pos':
                                        mapped_rel_data[field_idx] = rel_pos
                                    elif key == 'def_zh':
                                        mapped_rel_data[field_idx] = rel_def
                                    elif key in ['pronunciation', 'inflections', 'examples']:
                                        mapped_rel_data[field_idx] = ''
                            data.append(mapped_rel_data)
...
if __name__ == "__main__":
    ...
    result = fetcher.fetch(word, config)
    print("Result:", result)